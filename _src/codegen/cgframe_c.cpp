...
CGMem *CGFrame_C::tmpMem( int type ){
	if( !tmp_mem ){
		local_sz+=8;
		tmp_mem=-local_sz;
	}
...
CGMem *CGFrame_C::optMem( CGMem *e,char *buf ){
...
bool CGFrame_C::optMov( CGExp *lhs,CGExp *rhs ){
...
CGReg *CGFrame_C::genExp( CGExp *e ){
	if( CGReg *t=e->reg() ){
		return t;
...
CGMem *CGFrame_C::genMem( CGMem *e,char *buf ){
...
CGExp *CGFrame_C::genExp( CGExp *e,char *buf,int mask ){
...
CGReg *CGFrame_C::genLoad( CGMem *e ){
...
void CGFrame_C::genCopy( CGReg *r,CGReg *t ){
	if( r->id==t->id ) return;

	gen( mov(r,t),"\tmov\t'%i,'%i\n",r->id,t->id );
}

void CGFrame_C::genMov( CGExp *lhs,CGExp *rhs ){
...
CGReg *CGFrame_C::genCvt( CGCvt *t ){
	CGReg *r;
...
CGReg *CGFrame_C::genUop( CGUop *t ){
...
CGReg *CGFrame_C::genScc( CGScc *t ){
...
CGReg *CGFrame_C::genSym( CGSym *t ){
	CGReg *r=reg(t->type);
	gen( mov(r,t),"\tmov\t'%i,%s\n",r->id,t->value.c_str() );
	return r;
}

CGReg *CGFrame_C::genFrm( CGFrm *t ){
	CGReg *r=reg(CG_PTR);
	genMov( r,ebp );
	return r;
}

void CGFrame_C::genPush( CGExp *e ){
...
void CGFrame_C::genPush4( CGExp *e ){
...
void CGFrame_C::genPush8( CGExp *e ){
	CGReg *r=genExp( e );
	gen( xop(XOP_PUSH8,0,r),"\tpush\t'%i\n",r->id );
}

void CGFrame_C::genPop( CGExp *e ){
...
void CGFrame_C::genBcc( int cc,CGExp *lhs,CGExp *rhs,CGSym *tgt ){
...
CGMem *CGFrame_C::allocLocal( int type ){
	int n=(type==CG_FLOAT64 || type==CG_INT64) ? 8 : 4;
	local_sz+=n;
	return mem(type,ebp,-local_sz);
}

CGExp *CGFrame_C::allocSpill( CGReg *r ){
	int sz=8;
	if( fun->self ){
